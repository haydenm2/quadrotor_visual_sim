...
class kalman_filter:
    def __init__(self, Ts):
        ...
        self.B = CTRL.B
        self.C = CTRL.C
        self.L, _, _ = control.lqe(CTRL.A, np.eye(9), CTRL.C, SENSOR.Q_N, SENSOR.R_N)
        self.Ts = Ts

    def update(self, estimated_state, measurement, inputs):
        xhat = np.array((estimated_state.u, estimated_state.v, estimated_state.w, estimated_state.phi, estimated_state.theta, estimated_state.psi, estimated_state.p, estimated_state.q, estimated_state.r)).reshape(-1, 1)
        y = np.array((measurement.u, measurement.v, measurement.w, measurement.phi, measurement.theta, measurement.psi, measurement.p, measurement.q, measurement.r)).reshape(-1, 1)
        u = inputs.reshape(-1, 1)
        xhat_k = self.propogate(xhat, u, y)
        estimated_state.u = xhat_k.item(0)
        estimated_state.v = xhat_k.item(1)
        estimated_state.w = xhat_k.item(2)
        estimated_state.phi = xhat_k.item(3)
        estimated_state.theta = xhat_k.item(4)
        estimated_state.psi = xhat_k.item(5)
        estimated_state.p = xhat_k.item(6)
        estimated_state.q = xhat_k.item(7)
        estimated_state.r = xhat_k.item(8)
