...
class quad_viewer():
    def __init__(self, targets):
        ...
        self.draw_targets(targets.get_targets())
    ...
    def draw_targets(self, targets):
        ...
        size = np.array([10])
        colors = np.row_stack((red, green, blue))
        sizes = np.tile(size, len(targets[0]))
        self.target = gl.GLScatterPlotItem(pos=p_enu.T,
                                           color=colors,
                                           size=sizes)
        self.window.addItem(self.target)  # add body to plot
