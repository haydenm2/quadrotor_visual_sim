...
class controller:
    ...
    def update(self, cmd, state):
        x = np.array([[state.u - cmd.u, state.v - cmd.v, state.w - cmd.w, state.phi, state.theta, state.psi - cmd.psi, state.p - cmd.p, state.q - cmd.q, state.r - cmd.r]]).T  # using beta as an estimate for v
